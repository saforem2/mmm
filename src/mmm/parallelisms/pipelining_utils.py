...
def generate_split_points(job_config, pp_dim, model_config):
    from torch.distributed.pipelining.schedules import (  # type: ignore
    ...
    if total_stages > num_layers:
        raise ValueError(
            'Total stages cannot be greater than the number of layers'
        )
    ...
    logger.info(
        f"No 'pipeline_parallel_split_points' so the generated splits are: "
        f"{splits} This may be sub-optimal as the number of layers "
        "per stage may be unbalanced."
    )
    return splits
...
def stage_ids_this_rank(
    ...
    stages_per_rank = int(num_stages // pp_size)
    if style == 'loop':
        return tuple(
            int(pp_rank + s * pp_size) for s in range(stages_per_rank)
        )
    elif style == 'v':
        assert (
            stages_per_rank == 2
